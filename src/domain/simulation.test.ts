...
describe('world simulation',()=>{
 ...
 it('completes a compound chain and rewards once',()=>{let s=createInitialState(generateCharacter(1));const a=ANOMALIES[0]!;s={...s,player:a.position,character:{...s.character,gift:{id:'echo',name:'Echo',description:''}}};let r=activateGift(s,100);expect(r.changed).toBe(true);s={...r.state,borrowedGift:'mend'};r=activateGift(s,200);expect(r.state.seeds).toContain('singing-tree');expect(r.state.wardrobe).toContain('canvas-pack');const again=activateGift(r.state,300);expect(again.state.seeds.filter(x=>x==='singing-tree')).toHaveLength(1)});
