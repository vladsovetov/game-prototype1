...
export type TutorialStep='wake'|'move'|'gift'|'resonate'|'combine'|'plant'|'remember'|'personalize'|'done';
export interface TutorialState{step:TutorialStep;targetAnomalyId:string;borrowedGift:GiftId;start:Point}
export interface GameState{version:1;character:Character;player:Point;anomalies:Record<string,number>;discoveries:string[];seeds:string[];plantings:Record<string,string>;rewarded:string[];borrowedGift?:GiftId;tutorial?:TutorialState;memoryDetails?:Record<string,string>;effects:{rootedUntil:number;fragileUntil:number;fadingUntil:number;awakeProps:string[]};lastUpdated:number}
